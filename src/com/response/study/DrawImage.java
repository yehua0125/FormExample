...
public class DrawImage extends HttpServlet {
	...
	public static final int STRINGNUM=4;
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)
		...
		g.fillRect(0, 0, WIDTH, WIDTH);//填充区域
		//4设置图片的边框
		g.setColor(Color.red);//设置边框颜色
		g.drawRect(1, 1, WIDTH - 2, HEIGHT - 2);//设置边框区域
		//5在图片上画干扰线
		drawRandomLine(g);
		//6.写在图片上随机数
//		String random=drawRandomNum((Graphics2D) g, "ch");//生成中文验证图片
//		String random=drawRandomNum((Graphics2D) g, "nl");//生成数字和字母验证图片
//		String random=drawRandomNum((Graphics2D) g, "n");//生成数字验证图片
//		String random=drawRandomNum((Graphics2D) g, "l");//生成字母验证图片		
		//根据客户端传递的createTypeFlag标识生成验证图片
		...
		//response.setHeader("refresh", "3");//没3秒刷新一次
		//10将图片写给浏览器
